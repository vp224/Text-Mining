...
object MeaningfulPhraseFinding {
  def main(args: Array[String]) {
    ...
    val phraseWeight = 1.0 //how much weight do we give to phraseness in the final score
    val infoWeight = 100.0   //how much weight do we give to informativeness in the final score
    val numBigrams = 50     //how many bigrams to include in the final result (top K bigrams)
    val numMappers = 4     //how many mappers to use
    val numReducers = 4     //how many reducers to use
    val fgYear = 1960         //what is the year that we are considering to be the foreground (FG)
    val bigramsPath = "in.txt"           //the path to the bigrams file
    val unigramsPath = "in.txt"          //the path to the unigrams file
